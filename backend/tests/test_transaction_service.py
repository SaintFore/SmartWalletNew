...
def test_get_monthly_summary_basic(session: Session):
    ...
    assert result.total_expense == Decimal("150.0")
    assert result.total_income == Decimal("10000.0")
    assert result.net == Decimal("9850.0")
...
def test_get_monthly_summary_category_details(session: Session):
    ...
    food_summary = next((c for c in sorted_categories if c.category_name == "餐饮"), None)
    assert food_summary is not None
    assert food_summary.total == Decimal("130.0")
    assert food_summary.count == 2
    assert food_summary.category_icon == "🍜"

    transport_summary = next((c for c in sorted_categories if c.category_name == "交通"), None)
    assert transport_summary is not None
    assert transport_summary.total == Decimal("20.0")
    assert transport_summary.count == 1
    assert transport_summary.category_icon == "🚗"

    salary_summary = next((c for c in sorted_categories if c.category_name == "工资"), None)
    assert salary_summary is not None
    assert salary_summary.total == Decimal("10000.0")
...
def test_get_monthly_summary_december(session: Session):
    ...
    assert result.total_expense == Decimal("500.0")
    assert result.total_income == Decimal("0")
    assert result.net == Decimal("-500.0")
    assert len(result.by_category) == 1
    assert result.by_category[0].category_name == "餐饮"
    assert result.by_category[0].total == Decimal("500.0")
...
def test_get_monthly_summary_only_expense(session: Session):
    ...
    assert result.total_expense == Decimal("130.0")
    assert result.total_income == Decimal("0")
    assert result.net == Decimal("-130.0")
    assert len(result.by_category) == 1
...
def test_get_monthly_summary_only_income(session: Session):
    ...
    assert result.total_expense == Decimal("0")
    assert result.total_income == Decimal("10000.0")
    assert result.net == Decimal("10000.0")
    assert len(result.by_category) == 1
...
def test_transfer_not_included_in_summary(session: Session):
    """测试转账不计入收支汇总。"""
    account_id = create_account_id(session)
    food_category = Category(name="餐饮", icon="🍜")
    session.add(food_category)
    ...
    assert result.total_expense == Decimal("50.0")
    assert result.total_income == Decimal("0")
    # 转账的 1000 不应出现在任何汇总中
    assert len(result.by_category) == 1
    assert result.by_category[0].total == Decimal("50.0")
